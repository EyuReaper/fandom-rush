...
export default function SurvivalMode() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] text-white overflow-hidden relative selection:bg-gold selection:text-black">
      {/* Gold background glow for survival mode */}
      <div className={`absolute inset-0 transition-colors duration-500 pointer-events-none ${
        feedback === 'correct' ? 'bg-gold/10' : feedback === 'wrong' ? 'bg-red-500/10' : 'bg-gold/5'
      }`} />
      ...
      <div className="absolute top-0 left-0 right-0 z-50 px-6 py-6 flex justify-between items-center bg-gradient-to-b from-[#0a0a1a] to-transparent">
        <div className="flex items-center gap-6">
          ...
          {/* Escalation Level */}
          <div className="flex items-center gap-2 bg-gold/10 border border-gold/30 px-3 py-1.5 rounded-xl">
            <TrendingUp className="w-4 h-4 text-gold" />
            <span className="text-xs font-black text-gold">LV.{escalationLevel}</span>
          </div>

          {/* Banked Score */}
          <div className="flex items-center gap-2 bg-gold/10 border border-gold/30 px-3 py-1.5 rounded-xl">
            <Shield className={`w-4 h-4 ${isBankProtected ? "text-gold" : "text-gray-600"}`} />
            <span className={`text-xs font-black ${isBankProtected ? "text-gold" : "text-gray-600"}`}>
      ...
      <div className="h-full min-h-screen flex flex-col items-center justify-center pt-24 pb-12 px-6">
        <div className="w-full max-w-xl relative">
          ...
          <div className="absolute -top-20 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-2">
            <div className="flex items-center gap-2 px-4 py-1 bg-gold/10 border border-gold/20 rounded-full text-gold text-[9px] font-black tracking-[0.3em] uppercase">
                      ...
                      isSelected && feedback === "correct"
                        ...
                        : isSelected && feedback === "wrong"
                          ...
                          : isCorrectOption && feedback
                            ...
                            : "border-white/10 hover:border-gold hover:bg-white/10"
                    }`}
                  >
                    <span className="block text-xl md:text-2xl font-black italic tracking-tighter uppercase">{option}</span>
                    {isSelected && feedback === "correct" && (
                      <CheckCircle className="absolute bottom-4 right-4 text-gold w-6 h-6" />
                    )}
                    {isSelected && feedback === "wrong" && (
      ...
      <AnimatePresence>
        {floatingPoints && (
          ...
          >
            <span className="text-7xl md:text-9xl font-black italic text-gold drop-shadow-[0_0_30px_rgba(255,215,0,0.5)] tracking-tighter">
